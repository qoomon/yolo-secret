...
defineProps<{
  secret: { data: string, filename?: string, date?: Date },
}>();
...
<template>
  ...
  <v-text-field
      v-if="'filename' in secret && secret.filename"
      :model-value="secret.filename"
      :readonly="true"
      label="Secret File"
      variant="outlined"
      color="primary"
  ...
  >
    <template v-slot:append-inner>
      <v-icon
          color="primary"
          icon="mdi-download"
          @click="downloadBase64File(secret.data, secret.filename)"
      ></v-icon>
    </template>
  </v-text-field>
    ...
    <template v-slot:append-inner>
      ...
      >
        ...
        <v-icon
            ...
            @click="copyToClipboard(secret.data as string)
              .then(() => {
